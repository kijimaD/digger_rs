use crate::{raws::Reaction, Faction, Map, MyTurn, Position, WantsToEncounter};
use specs::prelude::*;
...
impl<'a> System<'a> for AdjacentAI {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (mut turns, factions, positions, map, mut wants_encounter, entities, player) = data;
        ...
        for (entity, _turn, my_faction, pos) in (&entities, &turns, &factions, &positions).join() {
            if entity != *player {
                ...
                if pos.y > 0 {
                    evaluate(idx - w as usize, &map, &factions, &my_faction.name, &mut reactions);
                }
                if pos.y < h - 1 {
                    evaluate(idx + w as usize, &map, &factions, &my_faction.name, &mut reactions);
                }
